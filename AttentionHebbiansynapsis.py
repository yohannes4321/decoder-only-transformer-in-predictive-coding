...
class AttentionHebbianSynapse(DenseSynapse):
    ...
    def __init__(self, name,  block_size,batch_size,dropout_rate, vocab_size, embed_dim,shape, eta=0., weight_init=None, bias_init=None,
    ...
    @staticmethod
    def _compute_attention(Q, K, V, mask, n_heads, d_head, dropout_rate, batch_size, key):
        """
        Compute multi-head attention.
        
        Inputs Q, K, V can be 2D (batch_size*seq_len, n_embed) or 3D (batch_size, seq_len, n_embed).
        Returns flattened 2D attention: (batch_size*seq_len, n_embed)
        """
        if Q.ndim == 2:
        ...
        score = jax.nn.softmax(score, axis=-1).astype(q.dtype)
        ...
        attention = attention.transpose(0, 2, 1, 3).reshape(B * S, -1)  # flatten to 2D
    ...
    def advance_state_attention(inputs_q,inputs_k,inputs_v, mask,n_heads,dropout_rate,weights,keys ,pre, post,  embed_dim, batch_size,
        ...
        """
        # Compute embeddings
        outputs = AttentionHebbianSynapse._compute_attention(
            inputs_q,inputs_k,inputs_v, mask,n_heads, embed_dim, dropout_rate,batch_size,keys       )
    ...
    def help(cls): ## component help function
        properties = {
            "synapse_type": "AttentionHebbianSynapse - performs an adaptable synaptic "
